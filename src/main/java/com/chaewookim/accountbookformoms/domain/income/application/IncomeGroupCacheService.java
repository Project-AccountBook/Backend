...
/**
 * 수입 그룹 평균 캐싱 wrapper. fixed/variable 각각 별도 캐시.
 */
@Service
@RequiredArgsConstructor
public class IncomeGroupCacheService {
    ...
    @Cacheable(
            cacheNames = RedisConfig.CACHE_GROUP_INCOME_AGE_FIXED,
    ...
    @Cacheable(
            cacheNames = RedisConfig.CACHE_GROUP_INCOME_AGE_VARIABLE,
            key = "T(java.util.Objects).hash(#startDate.toString(), #endDate.toString(), #birthFrom.toString(), #birthTo.toString())")
    ...
    @Cacheable(
            cacheNames = RedisConfig.CACHE_GROUP_INCOME_AMOUNT_FIXED,
    ...
    @Cacheable(
            cacheNames = RedisConfig.CACHE_GROUP_INCOME_AMOUNT_VARIABLE,
            key = "T(java.util.Objects).hash(#startDate.toString(), #endDate.toString(), #minAmount, #maxAmount)")
    ...
    @Cacheable(
            cacheNames = RedisConfig.CACHE_GROUP_INCOME_CATEGORY_FIXED,
    ...
    @Cacheable(
            cacheNames = RedisConfig.CACHE_GROUP_INCOME_CATEGORY_VARIABLE,
            key = "T(java.util.Objects).hash(#startDate.toString(), #endDate.toString(), #categoryId)")
